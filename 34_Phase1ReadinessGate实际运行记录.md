...
13. `TODO-071 / 线配筋生成旧逻辑证据与 P0 切片准备` 已形成 `E-IDA-045 / E-DEV-093`：本轮用 IDA MCP 复核 `sgroupbarline` 表项、`sub_1404DE720` 入口契约和 `sub_1404D10C0` 公共创建调用，并把单选、`sub_1405C6820 / sub_1405DA020` 选择 gate、内部子项下限、奇数索引实体抽取、4 个端点距离候选、初始最小距离候选 `10.0` 写入正式 `RebarGroupCreator` P0 raw/evidence；readiness gate 已把 `TODO-071` 纳入 done-report 映射并新增 `pending_after_docs` 拦截。当前 CTest 18/18 pass，readiness unit 31/31 pass，strict gate 84/84 pass，xhigh 只读 review 为 `needs_fix` 且 Important 已由主流程修复。当前下一步切到 `TODO-072 / 线配筋命令 handler P0：LegacySelection 到 RebarGroupCreator 事务接入`，不自动进入面配筋、接头、Excel、Detail 字段继续扩张或 golden。
