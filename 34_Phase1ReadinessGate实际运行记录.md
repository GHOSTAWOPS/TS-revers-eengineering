...
5. 后续开发仍按旧图石运行、IDA、SFL、Detail 证据闭合业务缺口。`TODO-080 / 线配筋公共创建 core 参数 gate 与 diagnostic 对齐 P0` 已纳入 readiness gate done-report 映射：要求存在 `118_M2-RebarCreate-010线配筋公共创建Core参数Gate与Diagnostic对齐P0实现记录.md`、`docs/phase1/app_build_reports/m2_rebar_create_010_run_001.md` 和同名 `.json`，并拦截 `pending_readonly_review` 这类未最终收口状态。该节点当时切到 `TODO-081 / 线配筋公共创建 createdPayload 与 objA/objB 字段语义补证 P0`，不自动进入完整线配筋算法、面配筋、弧筋、接头、Excel、Detail 或 golden。
...
22. `TODO-080 / 线配筋公共创建 core 参数 gate 与 diagnostic 对齐 P0` 已形成 `E-DEV-102`：`RebarGroupCreator` 新增 `LegacyPublicCreateGateSnapshot`，在几何查询前执行 `sub_1404D10C0` 已确认的 `objB / objA / sub_1405F25F0(objA) >= 3 / ENTITY_LIST count >= 1 / distanceA >= 0.002` gate，并把 gate 快照写入 raw evidence；readiness gate 已把 `TODO-080` 纳入 done-report 映射。该节点当时切到 `TODO-081 / 线配筋公共创建 createdPayload 与 objA/objB 字段语义补证 P0`，不自动进入完整线配筋算法、面配筋、弧筋、接头、Excel、Detail 或 golden。
23. `TODO-081 / 线配筋公共创建 createdPayload 与 objA/objB 字段语义补证 P0` 已形成 `E-IDA-048 / E-DEV-103`：IDA MCP 确认 `createdObject + 104` 可作为 `createdPayload`，`createdObject + 112` 只低置信记为 `linkedModelRef / createdLinkRef`，`sub_1405D5670` 第 4 个 double 来自 `distanceA_4digit`，且 `objA / objB` 在线配筋和弧形组入口会交换角色；readiness gate 已把 `TODO-081` 纳入 done-report 映射。当时下一步切到 `TODO-082 / 线配筋公共创建 roles DTO 与 raw evidence 对齐 P0`，不自动进入完整线配筋算法、面配筋、弧筋、接头、Excel、Detail 或 golden。
24. `TODO-082 / 线配筋公共创建 roles DTO 与 raw evidence 对齐 P0` 已形成 `E-DEV-104`：正式 `RebarGroupCreator` 新增 `LegacyPublicCreateRolesSnapshot`，把 `createdPayloadRef / linkedModelRef / distanceA_4digit / objA-objB role` 写入 `createdFromParameters / legacyRaw`，并让 `normalizeSegmentCurve` 使用 `distanceA_4digit`。readiness gate 已把 `TODO-082` 纳入 done-report 映射，要求默认 CTest、readiness gate、domain/rebar OCCT 泄漏检查和 xhigh 只读 review 全部闭合。当前下一步切到 `TODO-083 / 线配筋公共创建 sub_1405D5670 split-spline-trim 等价切片 P0`，不自动进入完整线配筋算法、面配筋、弧筋、接头、Excel、Detail 或 golden。
25. `TODO-083 / 线配筋公共创建 sub_1405D5670 split-spline-trim 等价切片 P0` 已形成 `E-DEV-105`：正式 `RebarGroupCreator` 新增 `LegacySegmentCurveNormalizeTrace / Result`，把 `api_entity_entity_distance / api_split_curve / api_curve_spline / endpoint trim` trace 写入 `createdFromParameters / legacyRaw`；`MainWindow` 的 viewer reader 通过 `OccLegacyGeometryAdapter` 生成 P0 summary trace，同时保持 `domain/rebar` 不依赖 OCCT / AIS。readiness gate 已把 `TODO-083` 纳入 done-report 映射，要求默认 CTest、readiness gate、domain/rebar OCCT 泄漏检查、todo 单 next、git diff check 和 xhigh 只读 review 全部闭合。当前下一步切到 `TODO-084 / 线配筋公共创建 sub_1405D5670 group-min-distance / dirty-write gap 切片 P0`，不自动进入完整线配筋算法、面配筋、弧筋、接头、Excel、Detail 或 golden。
