...
7. `TODO-065 / 生成工程图与下料表旧图石运行确认清单与工件门禁准备 P0` 已形成 `E-DEV-087`：新增 `docs/phase1/runtime_capture/todo_065_generate_package_and_schedule/` 模板目录和 `capture_notes` 模板，把生成工程图 / 下料表的运行确认清单收紧到输出目录 listing / `SHA256` / 最后修改时间 / rerun 覆盖行为 / 额外运行时弹窗 / 拒收伪工件规则；同时明确 `UnCutSteel.TXT` 只算可选旁证，不能单独证明 `生成工程图` 成功。当前下一步切到 `TODO-066 / 生成工程图与下料表旧图石真实运行工件回填 P0`，继续等真实旧图石工件回填，不自动启动旧图石、不声明 AutoCAD L2 或 golden 通过。
