...
- `TODO-041 / M2-Drawing-010` DetailWriter line-containers AutoCAD L2 运行确认准备 P0 已落地：复用 `line-containers` 独立变量生成 TODO-041 三图纸 Detail 包和手工 L2 确认清单，记录 FDrawing 插件 hash、旧样例包 hash，并再次确认本机未发现 `acad.exe / accoreconsole.exe / AutoCAD registry`，所以 `autocadL2=not_run`。该阶段不是 AutoCAD L2 通过、旧插件接受新包、真实连续线 / 隐藏线 / 中心线 / 填充线算法、完整工程图或 golden 证明。
- `TODO-042 / M2-Drawing-011` DetailWriter pointStb / FaceEdge AutoCAD L2 运行确认准备 P0 已落地：复用 `point-face-edge` 独立变量生成 TODO-042 三图纸 Detail 包和手工 L2 确认清单，记录 FDrawing 插件 hash、旧样例包 hash，并再次确认本机未发现 `acad.exe / accoreconsole.exe / AutoCAD registry`，所以 `autocadL2=not_run`。该阶段不是 AutoCAD L2 通过、旧插件接受新包、点筋生成规则、FaceEdge 生成规则、真实工程图算法或 golden 证明。
...
- `TODO-044 / M2-Drawing-013` DetailWriter Others / steeljoint-line AutoCAD L2 运行确认准备 P0 已落地：复用 `others-steeljoint` 独立变量生成 TODO-044 三图纸 Detail 包和手工 L2 确认清单，记录 FDrawing 插件 hash、旧样例包 hash、新包 hash，并再次确认本机未发现 `acad.exe / accoreconsole.exe / AutoCAD registry`，所以 `autocadL2=not_run`。该阶段不是 AutoCAD L2 通过、旧插件接受新包、真实接头线算法、Others 几何算法、完整工程图或 golden 证明。
...
5. 后续开发仍按旧图石运行、IDA、SFL、Detail 证据闭合业务缺口；`TODO-029 / Rebar.Edit.Move P0`、`TODO-032 / Rebar.Edit.Copy P0`、`TODO-030 / M2-Stats-001`、`TODO-031 / M2-Drawing-001`、`TODO-033 / M2-Drawing-002`、`TODO-034 / M2-Drawing-003`、`TODO-035 / M2-Drawing-004`、`TODO-036 / M2-Drawing-005`、`TODO-037 / M2-Drawing-006`、`TODO-038 / M2-Drawing-007`、`TODO-039 / M2-Drawing-008`、`TODO-040 / M2-Drawing-009`、`TODO-041 / M2-Drawing-010`、`TODO-042 / M2-Drawing-011`、`TODO-043 / M2-Drawing-012` 和 `TODO-044 / M2-Drawing-013` 已纳入 done-report 映射。下一步建议按 `TODO-045 / M2-Drawing-014` 先补真实接头线 / Others 几何算法证据，优先用 IDA MCP 或旧图石运行确认，不要在无证据时直接实现算法；用户能启动 AutoCAD 2020 时，仍可按 `69/72/73/74/75/77/78/80` 的手工清单继续 L2 导入确认；`TODO-026` golden 采集保持 pending，不自动进入。
