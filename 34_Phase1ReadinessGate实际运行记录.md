...
19. `TODO-077 / 线配筋旧图石运行确认清单与工件门禁 P0` 已形成 `E-DEV-099`：新增 `docs/phase1/runtime_capture/todo_077_line_group_ui_prompt_capture/` 模板目录、`capture_notes.md` 和 `collect_hashes.ps1`，把无选择、选错对象、有效对象、旧主参数窗口、公共 `Dialog #383`、状态栏 pane、模型树 / 输出前后状态、截图 / listing / hash / 操作步骤和拒收伪工件规则落成可执行采样清单；readiness gate 已把 `TODO-077` 纳入 done-report 映射并新增 pending 报告拦截测试。当前下一步切到 `TODO-078 / 线配筋旧图石运行确认工件回填 P0`，只接收和核对真实旧图石运行工件，不自动进入完整线配筋算法、面配筋、弧筋、接头、Excel、Detail 字段或 golden。
