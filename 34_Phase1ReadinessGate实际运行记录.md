...
5. 后续开发仍按旧图石运行、IDA、SFL、Detail 证据闭合业务缺口；`TODO-029 / Rebar.Edit.Move P0`、`TODO-032 / Rebar.Edit.Copy P0`、`TODO-030 / M2-Stats-001`、`TODO-031 / M2-Drawing-001`、`TODO-033 / M2-Drawing-002` 和 `TODO-034 / M2-Drawing-003` 已纳入 done-report 映射。下一步建议按 `TODO-035 / M2-Drawing-004` 做 DetailWriter 复杂字段骨架 P0，或在用户能启动 AutoCAD 2020 时继续执行 `69` 的手工 L2 导入清单；`TODO-026` golden 采集保持 pending，不自动进入。
