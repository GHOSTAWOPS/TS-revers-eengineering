...
5. 后续开发仍按旧图石运行、IDA、SFL、Detail 证据闭合业务缺口；`TODO-029 / Rebar.Edit.Move P0`、`TODO-032 / Rebar.Edit.Copy P0`、`TODO-030 / M2-Stats-001`、`TODO-031 / M2-Drawing-001`、`TODO-033 / M2-Drawing-002`、`TODO-034 / M2-Drawing-003`、`TODO-035 / M2-Drawing-004`、`TODO-036 / M2-Drawing-005`、`TODO-037 / M2-Drawing-006`、`TODO-038 / M2-Drawing-007`、`TODO-039 / M2-Drawing-008`、`TODO-040 / M2-Drawing-009`、`TODO-041 / M2-Drawing-010`、`TODO-042 / M2-Drawing-011`、`TODO-043 / M2-Drawing-012`、`TODO-044 / M2-Drawing-013`、`TODO-045 / M2-Drawing-014`、`TODO-046 / M2-Drawing-015` 和 `TODO-047 / M2-Drawing-016` 已纳入 done-report 映射。`TODO-048 / 旧图石启动阻塞提示链补证 P0` 已用 `E-IDA-030 / E-DEV-070` 静态闭合启动主链、`41 -> 许可已过期`、其他非 `0` 失败码回落到 `请检查网线是否接好` 和用户手工解除前置条件清单；`TODO-049` 继续补入 `E-RUN-004` 后已经闭环：用户确认旧图石在 USB 狗插入后可进入主界面、可打开 `SFL`、不需要内网，但仍需本地 HASP 安装条件。`TODO-050 / 旧图石非空接头线 Others 运行样例采集 P0` 当前已形成 `E-IDA-032 / E-DEV-072` 采样准备和阻塞记录：用户现场 USB 狗 / HASP 运行条件尚未满足，因此不能声明非空样例已采到。`TODO-051 / Others symbolcutIOS producer 与接头 UI 触发静态补证 P0` 已形成 `E-IDA-033 / E-DEV-073`：静态 producer 链已追到 `sub_14060C940 -> sub_14060A810 -> HVIEWPORT +840/+848 -> sub_14061F970 -> sub_14053A3F0`。`TODO-052 / 接头 UI caption / Ribbon 绑定静态资源补证 P0` 已形成 `E-IDA-034 / E-DEV-074`：接头内部 command id / handler 表更完整，`sub_1406F37B0` Ribbon 构造函数未直接命中接头命令 ID，Dialog #427/#428 只证明参数字段存在；`TODO-053 / 接头 context menu / command dispatch 静态深追 P0` 已形成 `E-IDA-035 / E-DEV-075`：动态 popup/context menu 机制、`sub_1405C2EF0 / sub_1406BA690` 和 `LoadMenuW` 资源路径已确认，但全量 MENU 资源和 known joint command immediate 仍不能闭合旧 UI caption / 右键菜单项绑定。当前 strict readiness 仍应通过。下一步建议切到 `TODO-054 / 接头 handler 业务对象筛选链静态分类 P0`，继续用 IDA MCP 缩小各接头 handler 内部对象筛选和参数入口缺口；用户能启动 AutoCAD 2020 时，仍可按 `69/72/73/74/75/77/78/80/81/82/83/84/85/89` 的手工清单继续 L2 导入确认；`TODO-026` golden 采集保持 pending，不自动进入。
