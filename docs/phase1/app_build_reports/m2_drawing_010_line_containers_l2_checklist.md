# M2-Drawing-010 Line-Containers AutoCAD L2 手工确认清单

## 目的

本清单用于在可启动 AutoCAD 2020 的环境中确认旧 FDrawing 插件是否接受 `TODO-041` 的 `line-containers` Detail 包。
...
```text
C:\Users\ghost\Desktop\reverse_engineering\【03】图石软件\【图石钢筋1比1复刻】\docs\phase1\app_build_reports\m2_drawing_010_detail_package
```
...
本包专门观察以下 XML 容器是否会导致旧插件导入失败：

```text
continue-line / lines / Line1
hidden-line / lines / Line1
central-line / lines / Line1
hatch-line / lines / Line1
```
...
```text
旧样例包失败
  -> 不判断 TODO-041 新包质量；先处理 AutoCAD / FDrawing 环境。

旧样例包成功，TODO-041 新包失败
  -> 记录为 TODO-041 L2 fail；进入 GAP-DRAW-002 字段完整性 / 旧插件容忍度排查。

旧样例包成功，TODO-041 新包成功
  -> 可把本样本记为 line-containers AutoCAD L2 pass。
     仍不代表完整工程图、连续线算法、隐藏线算法、中心线算法、填充线算法、剖切线算法、接头线算法或 golden 完成。
```
...
TODO-041 package import = pass/fail
error text =
screenshot paths =
