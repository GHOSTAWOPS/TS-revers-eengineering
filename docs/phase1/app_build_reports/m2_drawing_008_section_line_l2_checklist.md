# M2-Drawing-008 Section-Line AutoCAD L2 手工确认清单

## 目的

本清单用于在可启动 AutoCAD 2020 的环境中确认旧 FDrawing 插件是否接受 `TODO-039` 的 `section-line` Detail 包。
...
```text
C:\Users\ghost\Desktop\reverse_engineering\【03】图石软件\【图石钢筋1比1复刻】\docs\phase1\app_build_reports\m2_drawing_008_detail_package
```
...
```text
旧样例包失败
  -> 不判断 TODO-039 新包质量；先处理 AutoCAD / FDrawing 环境。

旧样例包成功，TODO-039 新包失败
  -> 记录为 TODO-039 L2 fail；进入 GAP-DRAW-002 字段完整性 / 旧插件容忍度排查。

旧样例包成功，TODO-039 新包成功
  -> 可把本样本记为 section-line AutoCAD L2 pass。
     仍不代表完整工程图、剖切线算法、隐藏线、填充线、接头线或 golden 完成。
```
...
TODO-039 package import = pass/fail
error text =
screenshot paths =
