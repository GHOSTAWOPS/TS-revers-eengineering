# M2-Drawing-013 Others / Steeljoint-Line AutoCAD L2 手工确认清单

## 目的

本清单用于在可启动 AutoCAD 2020 的环境中确认旧 FDrawing 插件是否接受 `TODO-044` 的 `Others / steeljoint-line` Detail 包。
...
```text
C:\Users\ghost\Desktop\reverse_engineering\【03】图石软件\【图石钢筋1比1复刻】\docs\phase1\app_build_reports\m2_drawing_013_detail_package
```
...
```text
Others 为空容器。
steeljoint-line 只确认 joints 容器存在。
真实接头线生成规则未实现。
真实 Others 几何规则未实现。
```
...
```text
旧样例包失败
  -> 不判断 TODO-044 新包质量；先处理 AutoCAD / FDrawing 环境。

旧样例包成功，TODO-044 新包失败
  -> 记录为 TODO-044 L2 fail；进入 GAP-DRAW-002 字段完整性 / 旧插件容忍度排查。

旧样例包成功，TODO-044 新包成功
  -> 只有在用户或自动脚本记录真实 AutoCAD 操作步骤、截图/日志、输出文件和 hash 后，
     才能把该真实运行样本记为 Others / steeljoint-line AutoCAD L2 pass。
     仍不代表完整工程图、真实接头线算法、Others 几何算法、剖切线算法、
     隐藏线算法、填充线算法、点筋算法、FaceEdge 生成规则或 golden 完成。
```
...
TODO-044 package import = pass/fail
error text =
screenshot paths =
...
AutoCAD L2 已通过。
旧插件接受新包。
Others / steeljoint-line 字段组合被旧插件接受。
真实接头线算法完成。
Others 几何算法完成。
