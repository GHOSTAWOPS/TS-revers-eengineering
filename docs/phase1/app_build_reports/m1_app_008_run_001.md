# M1-App-008 Run 001

## 结论

```text
decision = m1-app-008-pass
```

本轮完成 `LegacyGeometryAdapter P2C`。

## 通过项

窄测试：

```text
legacy_geometry_adapter_tests.exe ..\..\123.stp
result = pass
```

默认 CTest：

```text
CTest 8 / 8 passed
Total Test time = 156.36 sec
```

测试项：

- `command_registry_tests`
- `legacy_selection_ref_tests`
- `app_smoke`
- `viewer_smoke_tests`
- `domain_rebar_boundary`
- `step_import_probe_123`
- `occ_selection_index_123`
- `legacy_geometry_adapter_123`

Readiness gate：

```text
M1-Formal-Ready
78 / 78 pass
0 error
0 warning
```

## Adapter P2C

新增：

```text
LegacyCurveInterval
edgeInterval
```

当前能力：

```text
edge parameter interval
interval length
interval start / end points
interval bbox from samples
interval sample points
reversed interval diagnostic
```

## 样本

```text
sample = C:/Users/ghost/Desktop/reverse_engineering/【03】图石软件/123.stp
size = 87746388 bytes
sha256 = 6a3c4f2044c2cc1f1123f0f58c61b2c869fbad3110a3585fdab49e69daf52a2a
```

## 边界

本轮没有实现钢筋生成。

本轮没有迁移父目录钢筋业务主链。

P2C 是曲线区间摘要，不是完整拓扑裁剪、offset 或扫掠。

OCCT 查询仍限制在：

```text
geometry/occ/legacy_adapter
```

`domain/rebar` 边界扫描通过。

## 下一步

```text
M1-App-009：LegacyGeometryAdapter P3A 已完成。
见 m1_app_009_run_001.md。
```
